...
inline void icl_write_buffer_offset(icl_buffer* buf, cl_bool blocking, size_t offset, size_t size, const void* source_ptr, icl_event* wait_event, icl_event* event) {
	cl_event* ev = NULL; cl_event* wait_ev = NULL; cl_uint num = 0;
	_icl_set_event(wait_event, event, &wait_ev, &ev, &num);

	cl_int err_code = clEnqueueWriteBuffer(buf->dev->queue, buf->mem, blocking, offset, size, source_ptr, num, wait_ev, ev);
	ICL_ASSERT(err_code == CL_SUCCESS, "Error writing buffer: \"%s\"",  _icl_error_string(err_code));
}
...
inline void icl_read_buffer_offset(const icl_buffer* buf, cl_bool blocking, size_t offset, size_t size, void* source_ptr, icl_event* wait_event, icl_event* event) {
	cl_event* ev = NULL; cl_event* wait_ev = NULL; cl_uint num = 0;
	_icl_set_event(wait_event, event, &wait_ev, &ev, &num);

	cl_int err_code = clEnqueueReadBuffer(buf->dev->queue, buf->mem, blocking, offset, size, source_ptr, num, wait_ev, ev);
	ICL_ASSERT(err_code == CL_SUCCESS, "Error reading buffer: \"%s\"",  _icl_error_string(err_code));
}
...
void icl_start_timer(icl_timer* timer){
	...
	timer->start = (time_int)ts.tv_sec * 1e9 + (time_int)ts.tv_nsec;	
#else
	struct timespec tmp;
...
double icl_stop_timer(icl_timer* timer){
	...
	ts.tv_sec = mts.tv_sec;
	ts.tv_nsec = mts.tv_nsec;
	stop = (time_int)ts.tv_sec * 1e9 + (time_int)ts.tv_nsec;
...
icl_kernel*  icl_create_kernel(icl_device* dev, const char* file_name, const char* kernel_name, const char* build_options, icl_create_kernel_flag flag) {
	...
	if ((kernel_name != NULL) && (*kernel_name != 0)) {
		...
		ICL_ASSERT(err_code == CL_SUCCESS, "Error creating kernel %s: \"%s\"", kernel_name, _icl_error_string(err_code));
		kernel->dev = dev;
	}
